...
class LoanService {
  ...
  async createLoan(loanData, userId) {
    try {
      // Validate that loanType exists
      const loanType = await prisma.loanType.findUnique({ where: { id: loanData.typeId } });
      if (!loanType) {
        const error = new Error('Invalid loan type');
        error.status = 400;
        throw error;
      }
      const loan = await prisma.loan.create({
        data: {
          typeId: loanData.typeId,
