...
class LoanController {
    ...
    static async apply(req, res, next) {
        try {
            ...
            // Expect typeId instead of type
            if (!loanData.typeId) {
                const error = new Error('typeId is required');
                error.status = 400;
                return next(error);
            }
    ...
    static async getStatus(req, res, next) {
        try {
            ...
            let responseData = {
                loan: {
                    id: loan.id,
                    type: loan.type ? { id: loan.type.id, name: loan.type.name, description: loan.type.description } : null,
                    amount: loan.amount,
                    status: loan.status,
